...
//    const { setAuthTokens } = useAuth();
   ...
   const formik = useFormik({
       ...
       /*Post method to Register user when submiting form*/
       onSubmit: async (e) => {
           await axios.post('/register', {name: e.name, email: e.email, password: e.password})
           .then(e => {
               history.push('/cryptoList');
               history.go();
           })
           .catch(err => {
   ...
   return (
       <form className={classes.formSignup} onSubmit={formik.handleSubmit}>
           ...
           <h2 className={classes.titleSignup}>Créer un compte</h2>
